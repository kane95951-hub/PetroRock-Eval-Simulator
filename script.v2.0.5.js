/* PetroRock-Eval Simulator v2.0.5 — moteur de simulation et d'analyse client-side.
...
const EXTRA_I18N = {
  fr: {
    ...
    'lab-scene-title': 'FOUR DE PYROLYSE · VUE EN COUPE', 'lab-scene-caption': 'Schéma 2D pédagogique · les signaux réagissent au cycle.', 'scene-idle': 'EN ATTENTE', 'scene-heating': 'CHAUFFE EN COURS', 'scene-paused': 'EN PAUSE', 'scene-done': 'CYCLE TERMINÉ', 'scene-cell': 'CELLULE CHAUDE', 'scene-detector': 'LECTURE DES SIGNAUX', 'scene-control': 'RÉGULATEUR', 'scene-target': 'CIBLE · 600 °C', 'scene-gas': 'LIGNE DE GAZ · SCHÉMA', 'scene-analyser': 'ANALYSEUR', 'scene-axis': 'TEMPÉRATURE →', 'scene-signal': 'SIMULÉ', 'scene-svg-title': 'Four de pyrolyse et lecture des signaux', 'scene-svg-desc': 'Schéma animé du four, de l’échantillon rocheux, de la conduite des gaz et des signaux S1, S2 et S3.'
  },
  en: {
    ...
    'lab-scene-title': 'PYROLYSIS FURNACE · CUTAWAY', 'lab-scene-caption': 'Educational 2D schematic · signals respond to the cycle.', 'scene-idle': 'STANDBY', 'scene-heating': 'HEATING', 'scene-paused': 'PAUSED', 'scene-done': 'CYCLE COMPLETE', 'scene-cell': 'HEATED CHAMBER', 'scene-detector': 'SIGNAL READOUT', 'scene-control': 'CONTROLLER', 'scene-target': 'SETPOINT · 600 °C', 'scene-gas': 'GAS LINE · SCHEMATIC', 'scene-analyser': 'ANALYZER', 'scene-axis': 'TEMPERATURE →', 'scene-signal': 'SIMULATED', 'scene-svg-title': 'Pyrolysis furnace and signal readout', 'scene-svg-desc': 'Animated schematic of the furnace, rock sample, gas line and S1, S2 and S3 signals.'
  }
};

const GUIDE_I18N = {
  fr: {
    button: 'Guide', open: 'Ouvrir le guide de démarrage', close: 'Fermer le guide', eyebrow: 'GUIDE DE DÉMARRAGE',
    title: 'Bien démarrer en 1 minute', intro: 'Suis les quatre onglets de gauche à droite. Tu peux revenir en arrière à tout moment.',
    'step1-title': 'Saisis ton scénario', 'step1-copy': 'Choisis un type de matière, renseigne TOC, S1, HI, OI et Tmax, puis génère le scénario. S1 se règle séparément.', 'step1-link': 'Ouvrir les entrées',
    'step2-title': 'Observe le pyrogramme', 'step2-copy': 'Les courbes montrent S1, S2 et S3. En mode détecté, Tmax est lu au sommet de S2.', 'step2-link': 'Voir les courbes',
    'step3-title': 'Lis les résultats', 'step3-copy': 'Consulte HI, OI, PI, PY et les alertes QC. Une alerte indique un point à vérifier, pas un diagnostic.', 'step3-link': 'Voir les résultats',
    'step4-title': 'Analyse tes données', 'step4-copy': 'Importe un fichier CSV, TSV ou XLSX, vérifie les colonnes puis valide. Tu peux exporter le tableau ou préparer un rapport PDF.', 'step4-link': 'Importer des données',
    note: 'Ces simulations servent à apprendre. Vérifie toujours les unités et le contexte de tes données.', done: 'Compris, commencer'
  },
  en: {
    button: 'Guide', open: 'Open the getting started guide', close: 'Close the guide', eyebrow: 'GETTING STARTED',
    title: 'Get started in 1 minute', intro: 'Follow the four tabs from left to right. You can go back at any time.',
    'step1-title': 'Set up a scenario', 'step1-copy': 'Choose an organic matter preset, enter TOC, S1, HI, OI and Tmax, then generate the scenario. S1 is set independently.', 'step1-link': 'Open inputs',
    'step2-title': 'Read the pyrogram', 'step2-copy': 'The curves show S1, S2 and S3. In detected mode, Tmax is read at the S2 peak.', 'step2-link': 'View the curves',
    'step3-title': 'Review the results', 'step3-copy': 'Check HI, OI, PI, PY and QC alerts. An alert marks something to review; it is not a diagnosis.', 'step3-link': 'View results',
    'step4-title': 'Analyse your data', 'step4-copy': 'Import a CSV, TSV or XLSX file, check the column mapping, then apply it. Export the table or prepare a PDF report.', 'step4-link': 'Import data',
    note: 'These simulations are for learning. Always check the units and context of your data.', done: 'Got it, start'
  }
};
...
function applyLanguage(lang) {
  ...
  $$('[data-guide-i18n]').forEach(el => { el.textContent = GUIDE_I18N[state.lang][el.dataset.guideI18n]; });
  $('#guide-toggle').setAttribute('aria-label', GUIDE_I18N[state.lang].open);
  $('#guide-close').setAttribute('aria-label', GUIDE_I18N[state.lang].close);
...
const guideDialog = $('#quick-guide');
$('#guide-toggle').addEventListener('click', () => guideDialog.showModal());
$('#guide-close').addEventListener('click', () => guideDialog.close());
$('#guide-done').addEventListener('click', () => guideDialog.close());
guideDialog.addEventListener('close', () => storageSet('petrorock-guide-seen-v1', true));
$$('[data-guide-pane]').forEach(button => button.addEventListener('click', () => {
  guideDialog.close();
  activatePane(button.dataset.guidePane);
}));
...
function updateLabScene() {
  const scene = $('#lab-scene');
  const temperature = [250, 300, 400, 450, 600][Math.min(state.heatIndex, 4)];
  scene.dataset.stage = String(state.heatIndex);
  scene.classList.toggle('is-running', Boolean(state.animationTimer) && !state.animationPaused);
  scene.classList.toggle('is-paused', state.animationPaused);
  ['s1', 's2', 's3', 'tmax'].forEach((stage, index) => scene.classList.toggle(`stage-${stage}`, state.heatIndex >= index + 1));
  $('#lab-scene-temp').textContent = String(temperature);
  const status = state.animationPaused ? 'scene-paused' : state.animationTimer ? 'scene-heating' : state.heatIndex >= 4 ? 'scene-done' : 'scene-idle';
  $('#lab-scene-state').textContent = tr(status);
  const metricTmax = Number(state.metrics?.tmax);
  const targetTmax = Number($('#tmax-target').value);
  const tmax = Number.isFinite(metricTmax) ? metricTmax : Number.isFinite(targetTmax) ? targetTmax : 435;
  const markerX = 562 + (Math.max(TEMP_MIN, Math.min(TEMP_MAX, tmax)) - TEMP_MIN) / (TEMP_MAX - TEMP_MIN) * 143;
  $('#lab-scene-tmax-marker').setAttribute('x1', markerX.toFixed(1));
  $('#lab-scene-tmax-marker').setAttribute('x2', markerX.toFixed(1));
  $('#lab-scene-tmax-label').setAttribute('x', Math.min(677, markerX + 4).toFixed(1));
  $('#lab-scene-tmax-label').textContent = `Tmax ${format(tmax, 0)}°`;
}
...
function runLab() {
  ...
  if (state.animationPaused) { state.animationPaused = false; $('#pause-lab').textContent = tr('pause'); state.animationTimer = setInterval(advanceLab, 650); updateLabScene(); return; }
...
if (!storageGet('petrorock-guide-seen-v1', false)) guideDialog.showModal();
